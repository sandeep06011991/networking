...
public class ChannelEmulator {
	...
	private static final int MAX_Q_SIZE = 16; // max number of outstanding segments per client
	...
	private static double LOSS = 0.1; // loss rate (0.1)
	private static double DELAY = 0.1; // delay in seconds (.1)
	private static double DELAY_DEV_RATIO = 1.0; // ratio of deviation to average delay
	private static double CORRUPTION = 0.02; // (0.01)probability of corruption of exactly one byte in each 100 byte block
	...
	private static enum Commands {
		CONN, NAME, OK, QUIT, LIST, CHNL, PARAMS, LOSS, DELAY, CORRUPTION, DELAY_DEV_RATIO
	};
	...
	private static Logger log =
			Logger.getLogger(ChannelEmulator.class.getName());
			...
			for (InetSocketAddress isaddr : sockToCinfo.keySet()) {
				...
				if (cinfo != null &&
						cinfo.lastActive < System.currentTimeMillis() -
								MAX_INACTIVE_TIME) cleanup(isaddr);
			}
		}
	}
	...
	private byte[] corrupt(byte[] msg, ChannelInfo cinfo) {
		...
		for (int i = 0; i < msg.length; i += 100) {
			if (random.nextDouble() < corruption) {
				int j =
						(int) (random.nextDouble() * (Math.min(i + 100,
							msg.length - 1)));
	...
	private void delay(DatagramPacket dgram, ChannelInfo srcCinfo) {
		...
		double src_delay_dev_ratio =
				(srcCinfo != null ? srcCinfo.delay : DELAY);
		double dst_delay = dstCinfo != null ? dstCinfo.delay : DELAY;
		double dst_delay_dev_ratio =
				(dstCinfo != null ? dstCinfo.delay_dev_ratio : DELAY_DEV_RATIO);
		timer.schedule(
			task,
			Math.max(0, (long) ((dst_delay * (1 + dst_delay_dev_ratio *
	...
	private void mangle(DatagramPacket dgram, InetSocketAddress srcAddr)
		...
		if (msg != null && srcCinfo.getQSize() < MAX_Q_SIZE &&
				getTQSize() < MAX_TQ_SIZE) {
			srcCinfo.incrQSize();
			this.incrTQSize();
			dgram.setData(msg);
			delay(dgram, srcCinfo);
		}
		else log.fine("Dropping message from " + getNameString(srcAddr) +
	...
	private String processAsControlMessage(DatagramPacket dgram) {
		...
		InetSocketAddress isaddr =
				(InetSocketAddress) (dgram.getSocketAddress());
		...
		// first check for QUIT and happily clear away any state
			...
			sockToCinfo.put(isaddr, cinfo);
			response =
					Commands.OK.toString() +
							" Relaying to " +
							cinfo.peer +
							((peerSock == null || sockToCinfo.get(peerSock) == null) ? " who is probably offline"
									: (!cinfo.peer.equals(peerSock.toString()) ? " at " +
											peerSock
											: ""));
		}
		/*
		 ...
		 */
			...
			if (PRIVILEGED_MODE) { // will behave fine even with bad CHNL commands
				...
				allowCHNL(dgram) && (room = haveRoom(cinfo))) {
			response =
					Commands.OK.toString() + " " + Commands.CHNL.toString() +
							" " + printParams(cinfo);
				...
				(room = haveRoom(cinfo))) {
			response =
					Commands.OK.toString() + " " + Commands.LIST.toString() +
							" = ";
		...
		if (!room)
			response =
					"!" + Commands.OK +
							": chat server too crowded, try again later";
	...
	private String getPeerString(DatagramPacket dgram) {
		ChannelInfo cinfo = getChannelInfo(dgram);
		String peer =
				(cinfo != null ? (cinfo.peer != null ? cinfo.peer : "") : "");
	...
	private InetSocketAddress getPeerSock(String name) {
		...
		if (name != null && (isaddr = nameToSock.get(name)) == null) {
			...
			if (parts.length > 1) {
				try {
					InetAddress IP =
							InetAddress.getByName(parts[parts.length - 2]);
		...
		for (int i = 0; i + 1 < args.length; i += 2) {
			...
			try {
				if (args[i].equals("-PORT") || args[i].equals("-P")) {
					...
					else if (args[i].equals("-DELAY_DEV_RATIO") ||
							args[i].equals("-R")) {
						DELAY_DEV_RATIO = val;
					}
					else if (args[i].equals("-CORRUPTION") ||
							args[i].equals("-C")) {
						LOSS = val;
					}
				}
			} catch (NumberFormatException nfe) {
				log.warning("Bad " + args[i] +
						" input, ignoring or using default value");
				continue;
			}
		}
	}
	...
	private String printParams(ChannelInfo cinfo) {
		return " [ LOSS " +
				(cinfo != null ? cinfo.loss : LOSS) +
				"  DELAY " +
				(cinfo != null ? cinfo.delay : DELAY) +
				"  DELAY_DEV_RATIO " +
				(cinfo != null ? cinfo.delay_dev_ratio : DELAY_DEV_RATIO) +
				"  CORRUPTION " +
				(cinfo != null ? cinfo.corruption : CORRUPTION) +
